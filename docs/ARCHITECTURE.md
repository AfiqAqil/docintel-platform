...
| Meaningful outputs | Main stack: the application URL, the API's internal name, the bucket, both queue URLs, the database endpoint and its secret ARN (never the value), the cluster and service names and the target group ARN that the deploy pipeline's checks use, the log groups, and which LLM mode is deployed. Bootstrap: the state bucket, the ECR repository URLs, the OpenAI secret name, and the two CI role ARNs |
